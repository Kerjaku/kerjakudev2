import React, { Component } from 'react'

export default class ModuleSevenD extends Component {

    render() {
        return (
            <>
                <div class="module-page-title" id="moduleonename">
                    D. MENGENAL BUDAYA KERJA UNTUK MENGURANGI CULTURE SHOCK
                </div>
                <div class="module-text" id="moduleonetext">
                    Kunci keberhasilan mengatasi <i>culture shock</i> adalah persiapan yang baik dan rencana yang matang untuk menyesuaikan diri dengan budaya di lingkungan yang baru. Hal ini harus disertai dengan motivasi yang kuat untuk berhasil.
                    <br /><br />
                    Dengan demikian tahap paling awal untuk dilakukan adalah mempersiapkan diri dengan mencari informasi budaya kerja pada lingkungan baru yang akan dimasuki. Secara lebih terperinci, langkah-langkah yang bisa diaplikasikan adalah
                    <ol>
                        <li>Mencari informasi budaya kerja di lingkungan baru dan merekonsiliasi informasi yang kita dapatkan.Rekonsiliasi informasi dilakukan untuk mengelola pemikiran akan: </li>
                        <ul>
                            <li>Ide yang terbentuk sebelumnya</li>
                            <li>Harapan</li>
                            <li>Kebutuhan personal </li>
                            <li>Zona nyaman</li>
                            <li>Reaksi kita terhadap informasi</li>
                        </ul>
                        <br /><br />
                        <li>
                            Terapkan pola pikir positif. Tanamkan bahwa pola pikir positif adalah sesuatu yang menghibur. Bangun dan tanamkan pemikiran bahwa apa pun situasi yang Anda hadapi, Anda dapat mengandalkan diri sendiri untuk belajar dari situasi.
                            <br /><br />
                            Hal ini tentang konstruksi berpikir: berorientasi pada solusi dan pelajaran. Jangan menolak atau mengabaikan kesulitan. Sebaliknya, akui itu dan berkonsentrasilah untuk menghadapi hal tersebut.
                            <br /><br />
                            Sadari bahwa berurusan dengan kesulitan dapat dengan sendirinya menjadi hal yang sangat bermanfaat dan menyenangkan serta memperkaya pengalaman anda.
                        </li>
                        <br/><br/>
                        <li>
                            Mengubah perilaku anda dalam budaya baru.
                            <br />
                            Konteks - perilaku baru adalah cara berbeda untuk berbicara atau mengekspresikan diri, gerakan yang berbeda. Hal ini bisa mulai anda latih dan biasakan pada waktu yang tersedia. Contoh dari perilaku baru misalnya mulai membiasakan bangun pagi, melatih menyampaikan pendapat atau terkait teknikalitas.
                        </li>
                        <br/><br/>
                        <li>
                            <i>Kindness</i>
                            Kebaikan hati adalah krusial, semakin Anda bisa mengandalkan diri sendiri untuk menunjukkan kebaikan diri sendiri dan orang lain, semakin Anda merasa nyaman.
                        </li>
                    </ol>
                </div>
            </ >
        )
    }
}